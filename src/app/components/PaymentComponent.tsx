...
import { CurrentUser } from "../type/util";
...
function PaymentComponent({
  ...
  // Function to register the event booking
  const registerEvent = async (paymentId: string) => {
    try {
      await fetch("/api/user/book-event", {
        ...
        body: JSON.stringify({
          ...
          ticketHolders: Array.from({ length: ticketQuantity }, () => ({
            seatNumber: Math.floor(Math.random() * 100) + 1, // Random seat number for demo
            ticketCode: generateTicketCode(), // Generate a unique ticket code
          })),
          amount: discountAmount === 0 ? eventData?.price : discountAmount, // Use final price after discount
  ...
  async function displayRazorpay() {
    ...
    const options = {
      ...
      handler: async function (response: any) {
        try {
          await registerEvent(response.razorpay_payment_id);
          window.location.href = `/successPage?eventId=${eventData._id}&dbEventId=${eventData.eventId}&amount=${eventData.price}&transactionId=${response.razorpay_payment_id}&coupon=${appliedCoupon?.code || ""}&discount=${discountAmount}`;
        } catch (err) {
          const errorMessage =
            err instanceof Error && err.message
              ? err.message
